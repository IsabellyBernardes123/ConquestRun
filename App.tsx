...
import { 
  ...
  Clock, Menu, User as UserIcon, Navigation, 
...
const App: React.FC = () => {
  ...
  const handleStop = async () => {
    ...
    if (isClosed && currentPath.length > 5) {
      ...
      const { data: savedT } = await supabase
        .from('territories')
    ...
    setTimeout(() => setCurrentPath([]), 1000);
        ...
        {selectedTerritory && (
          <div className="absolute inset-0 z-[100] flex items-end lg:items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in duration-300">
            ...
            <div className="relative bg-[#0a0f18] border-t border-gray-800 lg:border lg:border-white/10 w-full max-w-2xl rounded-t-[3rem] lg:rounded-[3rem] overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-500">
              <div className="p-8">
                <h2 className="text-2xl font-black mb-4">{selectedTerritory.name}</h2>
                <p className="text-gray-400 mb-6 italic">{selectedTerritory.strategy}</p>
                <button onClick={() => setSelectedTerritory(null)} className="w-full py-5 bg-gray-800 rounded-2xl font-black uppercase tracking-widest">Fechar Relatório</button>
              </div>
            </div>
          </div>
        )}
          ...
          {isRecording && (
            <div className="bg-gray-950/90 backdrop-blur-3xl border border-white/10 p-6 rounded-[2.5rem] shadow-2xl flex justify-around items-center w-full max-w-lg mx-auto animate-in slide-in-from-bottom-20 duration-500">
              <div className="flex-1 text-center border-r border-white/5">
                ...
                <p className="text-3xl font-black text-blue-400 tracking-tighter">{(calculateDistance(currentPath) / 1000).toFixed(2)} km</p>
              </div>
              <div className="flex-1 text-center">
                ...
                <div className={`text-xs font-black flex items-center justify-center space-x-2 ${checkClosedLoop(currentPath) ? 'text-green-400' : 'text-yellow-400'}`}>
                  {checkClosedLoop(currentPath) ? <ShieldCheck size={16} /> : <Navigation size={16} className="animate-pulse" />}
                  <span className="uppercase tracking-widest">{checkClosedLoop(currentPath) ? 'FECHADO' : 'MAPEANDO'}</span>
                </div>
              </div>
            </div>
          )}
          <div className="flex items-center justify-center space-x-5 w-full max-w-lg mx-auto">
            <button className="w-16 h-16 bg-gray-950/80 backdrop-blur-2xl border border-white/10 rounded-[1.75rem] flex items-center justify-center text-gray-400 active:scale-90 transition-all">
              <MapPin size={24} />
            </button>
            {!isRecording ? (
              <button onClick={() => setIsRecording(true)} className="bg-blue-600 hover:bg-blue-500 text-white w-28 h-28 rounded-[2.5rem] shadow-[0_0_60px_rgba(37,99,235,0.6)] transition-all flex items-center justify-center hover:scale-105 active:scale-90">
                <Play size={44} fill="currentColor" className="ml-2" />
              </button>
            ) : (
              <button onClick={handleStop} className="bg-red-600 hover:bg-red-500 text-white w-28 h-28 rounded-[2.5rem] shadow-[0_0_60px_rgba(220,38,38,0.6)] transition-all flex items-center justify-center hover:scale-105 active:scale-90">
                <Square size={36} fill="currentColor" />
              </button>
            )}
            <button onClick={() => setIsSidebarOpen(true)} className="w-16 h-16 bg-gray-950/80 backdrop-blur-2xl border border-white/10 rounded-[1.75rem] flex items-center justify-center text-gray-400 active:scale-90 transition-all">
              <UserIcon size={24} />
            </button>
          </div>
        </div>
